...
class App_happy_birds(tk.Tk):
	...
	def click(self, mclick):
		if mclick.x>self.x_center: # si l'utilisateur place le projectile après l'absisse du lanceur
			print("ERROR POSITION")
		else:
			if self.solve != 0: # si un lancé est toujours en cours (boucle sur la fc move) on l'arrete
				self.after_cancel(self.solve)

			if self.first == False: # Si ce n'est pas le premier lancé effacer l'ancienne trajectoire et projectile
			...
			self.x0 = mclick.x
			self.y0 = mclick.y
			# Initialisation de l'angle et de la norme du vecteur (click souris - centre du lanceur)
	...
	def move(self, event=None):
		# MAJ de l'angle et de la norme du vecteur (projectile - centre du lanceur)
		...
		# Lorsque le prjectile est propulsé par le lanceur la force de rappel du lanceur est non nulle
		...
		# Calcule de l'accélération, de la vitesse pouis de la position du projectile
		...
		# Stopper le mouvement du projectile lorsqu'il disparait verticalement de l'écran
	...
	def restart(self):
		self.first = True
		if self.solve != 0 : # Stop la boucle "after" de la fc move si elle est lancée
			self.after_cancel(self.solve)
